// frontend/src/app/dashboard/admin/analytics/page.js
'use client';
...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    fetchAnalytics(); // No auth check needed here, AdminLayout handles it
  }, []);
              ...
              <ul className="space-y-4">
                {analyticsData.recentActivity.map((activity, index) => (
                  <li key={index} className="flex items-center text-gray-700 text-lg">
                    <span className="font-semibold w-32 flex-shrink-0">{activity.type}:</span>
                    <span className="flex-grow">{activity.detail}</span>
                    <span className="text-sm text-gray-500 ml-4">{new Date(activity.date).toLocaleString()}</span>
                  </li>
                ))}
              </ul>
            ) : (
